...
export default function Main({ classroom, player }: { classroom: Classroom; player: Player }) {
	...
	useEffect(() => {
		...
		const filterMaxedOutRepeatables = (repeatables: Repeatable[]) => {
			const filteredArray: Repeatable[] = []
		...
		onSnapshot(repeatableQuery, (snapshot) => {
			const repeatablesFetch = async () => {
				...
				snapshot.forEach((doc) => {
					if (doc.data().assigned?.includes(player.id)) {
						repeatable.push(Object.assign({ id: doc.id }, doc.data()) as Repeatable)
					}
				})
	...
	const handleTabChange = (event: React.SyntheticEvent, newTabIndex: 0 | 1 | 2 | 3 | 4) => {
		setPage(newTabIndex)
	}

	const handleIsRepChange = (event: React.SyntheticEvent, newTabIndex: 0 | 1) => {
				...
				<Table sx={{ minWidth: 650 }} aria-label='simple table'>
					...
					<TableBody>
                      ...
                      /> */}
										...
										<TableCell align='left'>{completeRepeatableButton(repeatable)}</TableCell>
									</TableRow>
							  ))}
					</TableBody>
				</Table>
