...
export async function deleteRepeatable(
  classroomID: string,
  repeatableID: string
) {
  await deleteDoc(
    doc(db, `classrooms/${classroomID}/repeatables/${repeatableID}`)
  );
}
...
export async function completeTask(
  ...
  const docRef = doc(
    db,
    `classrooms/${classroomID}/tasks/${taskID}/completionTimes/${playerID}`
  );
...
export async function addTask(classID: string, task: Task, teacherID: string) {
  ...
  await addDoc(collection(db, `classrooms/${classID}/tasks`), {
    ...
    assigned: classSnap
      .data()
      .playerList.filter((id: string) => id !== teacherID), // filter out the teacher's id
    completed: [],
    confirmed: [],
  });
}
...
export async function addRepeatable(
  ...
  // Update tasks collection
  const repeatableRef = await addDoc(
    collection(db, `classrooms/${classID}/repeatables`),
    {
  ...
  classSnap
    ...
    .forEach(async (playerID: string) => {
      await addDoc(
        collection(
          db,
          `classrooms/${classID}/repeatables/${repeatableRef.id}/lastRefresh`
        ),
        {
          id: playerID,
...
export async function purchaseItem(
  ...
  if (playerSnap.exists()) {
    ...
    const inv = collection(
      db,
      `classrooms/${classID}/players/${studentID}/inventory`
    );
    const invSnapshot = (await getDocs(inv)).docs;

    for (var i in invSnapshot) {
      if (
        invSnapshot[i].data().item_id === item.id &&
        invSnapshot[i].data().type === item.type &&
        (!invSnapshot[i].data().subtype ||
          invSnapshot[i].data().subtype === item.subtype)
      ) {
...
export async function denyRepeatable(
  ...
  const repeatableRef = doc(
    db,
    `classrooms/${classroomID}/repeatables/${repeatableID}`
  );
...
export async function refreshAllRepeatables(
  classroomID: string,
  playerID: string
) {
  ...
  const repeatablesRef = collection(
    db,
    `classrooms/${classroomID}/repeatables`
  );
...
async function refreshRepeatable(
  classroomID: string,
  playerID: string,
  repeatableID: string
) {
