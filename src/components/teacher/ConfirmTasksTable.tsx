...
export default function ConfirmTasksTable({
      ...
      const cTaskFetch = async () => {
        ...
        snapshot.forEach(async (doc) => {
          ...
          const completionsQuery = query(
            collection(
              db,
              `classrooms/${classroom.id}/tasks/${doc.id}/completionTimes`
            )
          );
      ...
      const cRepeatablesFetch = async () => {
        ...
        snapshot.forEach(async (doc) => {
          ...
          const completionsQuery = query(
            collection(
              db,
              `classrooms/${classroom.id}/repeatables/${doc.id}/playerCompletions`
            )
          );
  ...
  const formatStatus = (task: TaskWithCompletionTimes, playerID: string) => {
    ...
    const playerCompletion = completionTimes.filter(
      (completion) => completion.id === playerID
    );
            ...
            <TableBody>
              ...
              {completedTasks.map((task) => {
                return task.completed?.map((playerID) => {
                  const playersCompleted = playerData.filter(
                  ...
                  return playersCompleted.map((player) => (
                    ...
                    >
                      ...
                      <TableCell align="center">
                        <Button
                          onClick={() =>
                            confirmTask(classroom.id, player.id, task.id)
                          }
                          variant="contained"
                        >
                          Confirm
                        ...
                        <Button
                          onClick={() =>
                            denyTask(classroom.id, player.id, task.id)
                          }
                          variant="contained"
                          color="error"
                        >
